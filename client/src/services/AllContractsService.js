...
import {
  AllAddresses,
  getDepositAddresses,
} from "shared/contracts/contractsData";
...
class AllContracts {
  ...
  initAllEventsContracts = async () => {
    try {
      ...
      const promises = eventsAddresses.map(async (address) => {
        await this._initializeContractForEvent(address);
      });
      await Promise.all(promises);
    } catch (err) {
      console.log("initAllEventsContracts");
    }
  };
  ...
  initAllTeamsContracts = async () => {
    try {
      ...
      const promises = teamsAddresses.map(async (address) => {
        await this._initializeContractForTeam(address);
      });
      await Promise.all(promises);
    } catch (err) {
      console.log("initAllTeamsContracts");
    }
  };
